...
logger = create_logger("query_knowledges")  # Create a logger for this module

# Connect to the Chroma database and get the collection
collection = chroma_client.get_collection(name=config.KNOWLEDGE_COL)


def query(appliance, query_text, top_k, max_distance):
    """Perform similary search of query_text and return the top_k documents"""
    result = collection.query(
        query_texts=[query_text], n_results=top_k, where={"appliance": appliance}
    )
    documents = []
...
def query_to_context(appliance, query_text):
    """Perform similary search of query_text and convert to QA pairs of Q and A"""
    top_k = config.TOP_K
    max_distance = config.MAX_DIST
    documents = query(
        appliance, query_text=query_text, top_k=top_k, max_distance=max_distance
    )
    context = ""
...
if __name__ == "__main__":
    ...
    for test in tests:
        ...
        result = query(
            appliance=appliance, query_text=query_text, top_k=20, max_distance=2.0
        )
